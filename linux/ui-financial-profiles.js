...
function bindHome(panel,p){
  ...
  panel.querySelectorAll('[data-flp-profile]').forEach(function(b){b.onclick=function(){api().select(b.dataset.flpProfile)}});var change=panel.querySelector('[data-flp-open-selector]');if(change)change.onclick=function(){api().openSelector(false)}
}
var busy=false;
function render(){
