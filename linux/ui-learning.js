...
2:{
 ...
 http:E('curl -I http://server01.lab.test','HTTP接続を確認',['curl -I http://localhost','curl http://server01.lab.test']),
 https:E('curl -Ik https://server01.lab.test','HTTPS接続を確認',['curl -kI https://server01.lab.test','curl -k https://server01.lab.test']),
...
3:{
 ...
 pingname:E('ping -c 4 server01.lab.test','名前解決込みで疎通確認',['ping server01.lab.test']),
 curlip:E('curl -I http://192.168.1.20','IP直打ちでHTTP確認',['curl http://192.168.1.20']),
 curlname:E('curl -I http://server01.lab.test','名前解決込みでHTTP確認',['curl http://server01.lab.test']),
 ...
 all:E('ip addr; ip route; getent hosts server01.lab.test','IP・Route・名前解決をまとめて確認',['ip a; ip r; getent hosts server01.lab.test'])
},
4:{
 passwordLogin:E('ssh learner@192.168.1.20','Password認証でSSH接続',['ssh learner@server01.lab.test']),
 keyLogin:E('ssh -i ~/.ssh/id_ed25519 learner@192.168.1.20','公開鍵認証でSSH接続',['ssh -i ~/.ssh/id_ed25519 learner@server01.lab.test']),
...
function showInput(btn,e){
  panel.hidden=false;
  panel.innerHTML='<div class="mobile-learning-task"><b>MISSION</b><strong>'+esc(btn.textContent.trim())+'</strong></div><div class="mobile-engineer-prompt"><span>learner@server01:~$</span><input autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="Linuxコマンドを入力"><button type="button">実行</button></div><div class="mobile-learning-feedback">コマンドを自分で入力します。</div>';
