...
const HOME={
  ...
  cloud:{
    ...
    problem:'Server・Network・DB基盤を自社で購入・設置していると、必要な構成が揃うまでに時間も費用もかかる。',
...
const CLOUD_META={
  ...
  6:['Route / NAT','通信の向きと経路を決める部品','provisioned','外へ出る通信と、外から入る通信を別々の経路として設計するためです。','経路が定義されていないと、内部Serverから外部へ更新を取りに行けず、不要な着信経路も残る。'],
  ...
  11:['Managed Database','DB基盤の運用をProviderと分担するData管理service','provisioned','Patch・HA・Backupといった基盤運用をProviderへ任せ、Data設計とSQLに集中するためです。','自前でDB Serverを運用すると、Patch・冗長化・Backupまで自分たちの当番として抱えることになる。'],
