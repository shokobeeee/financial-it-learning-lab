...
const rows=[
 ...
 {key:'serverless',term:'Serverless Compute',layer:'compute',model:'serverless',plain:'Serverの台数・OS管理を強く意識せず、FunctionやRequest単位で処理を実行する方式。',why:'小さな処理やEvent駆動に向くが、実行時間・State・Cold Start・Vendor制約等も考える。',bank:'compute',lab:null,products:{aws:'AWS Lambda',gcp:'Cloud Run / Cloud Functions',azure:'Azure Functions',oci:'OCI Functions'},aliases:['lambda','cloud functions','cloud run','azure functions','oci functions','serverless']},
...
function openAtlas(key){const o=overlay();if(key&&rowMap[key])state.selected=key;o.classList.add('open');document.body.style.overflow='hidden';render()}
...
function injectLabStrip(){
 ...
 let s=hero.querySelector('.cloudAtlasLabStrip');if(!s){s=document.createElement('section');s.className='cloudAtlasLabStrip';const goal=hero.querySelector('.goal');if(goal)goal.insertAdjacentElement('afterend',s);else hero.appendChild(s)}
