...
const rows=[
 ...
 {key:'secret-key-cert',term:'Secret / Key / Certificate',layer:'identity',model:'operations',plain:'Password等のSecret、暗号鍵、TLS証明書を安全に保管・更新する仕組み。',why:'コードへの直書きや期限切れ、鍵の過剰権限を防ぐ。Secret・Key・Certificateは別物だが、ここでは「秘密情報をどう管理するか」という学習単位で並べる。',bank:'identity',lab:13,products:{aws:'Secrets Manager / KMS / ACM',gcp:'Secret Manager / Cloud KMS / Certificate Manager',azure:'Key Vault / Managed HSM',oci:'Vault / Certificates'},aliases:['secret','kms','key vault','acm','certificate','vault','secret manager','managed hsm']},
 ...
 {key:'saas',term:'SaaS（利用形態）',layer:'architecture',model:'concept',mappingMode:'examples',plain:'ApplicationそのものをServiceとして利用するCloud利用形態。',why:'自分でServerやApplicationを作らず使える一方、Data・Identity・設定・契約・Exit等の責任は残る。各社の例は用途が異なり、相互代替ではない。',bank:'architecture',lab:null,products:{aws:'用途別SaaS（サービスごとに異なる）',gcp:'例: Google Workspace等',azure:'例: Microsoft 365等',oci:'例: Oracle Fusion Cloud Applications等'},aliases:['saas','microsoft 365','google workspace']},
...
const LAB_META=[
 ...
 {key:'failure-domain',label:'障害範囲を分ける'}, {key:'object-storage',label:'Storageを使い分ける'}, {key:'managed-db',label:'Managed DB'}, {key:'iam',label:'誰が何を操作できる？'},
...
function launch(){
 ...
 if(top){const nav=top.querySelector('.nav');const b=document.createElement('button');b.type='button';b.className='cloudAtlasTopBtn';b.innerHTML='☁️ Cloud Map';b.onclick=()=>openAtlas(null,b);if(nav)nav.appendChild(b);else top.appendChild(b)}
 const f=document.createElement('button');f.id='cloudAtlasLaunch';f.type='button';f.className='cloudAtlasLaunch';f.innerHTML='☁️<span>Cloud Map</span>';f.onclick=()=>openAtlas(null,f);document.body.appendChild(f);
}
...
function overlay(){
 ...
 <div class="cloudAtlasPanel" role="dialog" aria-modal="true" aria-labelledby="cloudAtlasTitle">
  <header class="cloudAtlasHead"><div><div class="cloudAtlasKicker">CLOUD MAP / ZERO-BASE GLOSSARY</div><h2 id="cloudAtlasTitle">☁️ Cloudを「位置」で理解する</h2><p>用語を <b>何をする？ / どこまで任せる？ / 各社では何て呼ぶ？ / 銀行Systemのどこ？</b> の4軸で整理します。</p></div><button class="cloudAtlasClose" type="button" aria-label="閉じる">×</button></header>
...
function closeAtlas(){
 const o=document.getElementById('cloudAtlasOverlay');if(o)o.classList.remove('open');document.body.style.overflow='';
...
function renderModels(main){
 const examples={iaas:'EC2 / Compute Engine / Azure VM / OCI Compute',managed:'RDS / Cloud SQL / Azure SQL / Autonomous Database',serverless:'Lambda / Cloud Run / Azure Functions / OCI Functions',saas:'例: Microsoft 365 / Google Workspace / Oracle Fusion Cloud Applications（用途は別）',control:'IAM / Policy / Audit / Key管理',iaasFoundation:'VPC / VNet / VCN / Subnet / Route'};
 main.innerHTML=`<div class="modelIntro"><h3>「どこまでCloud側へ任せるか」で見る</h3><p>IaaS→Managed/PaaS→Serverless/SaaSになるほど、一般にServer・OS等を自分で管理する範囲は減ります。ただし<b>Data・Identity・設定・業務責任はゼロになりません。</b></p></div><div class="modelLadder"><div class="modelScale">自分で管理する範囲 多い ↑</div>${['iaasFoundation','iaas','managed','serverless'].map(k=>`<div class="modelStep"><b>${esc(MODELS[k].name)}</b><span>${esc(MODELS[k].desc)}</span><em>${esc(examples[k]||'')}</em></div>`).join('')}<div class="modelStep saas"><b>SaaS</b><span>ApplicationそのものをServiceとして利用する。Infrastructure/Application運用は大きくCloud側へ寄る。</span><em>${esc(examples.saas)}</em></div><div class="modelScale">↓ Cloud側へ任せる範囲 多い</div></div><div class="modelCross"><h3>横断Control Plane</h3><p>${esc(MODELS.control.desc)}</p><b>${esc(examples.control)}</b></div><div class="modelCross"><h3>分類軸を混ぜない</h3><p>Availability Zoneは障害範囲、Auto Scaling / MIG / VM Scale SetsはComputeの冗長化・台数制御。似た場面で使っても同じ分類ではありません。</p></div>`
}
...
function injectLabStrip(){
 ...
 const layer=LAYERS[row.layer],model=MODELS[row.model],product=productFor(row);
